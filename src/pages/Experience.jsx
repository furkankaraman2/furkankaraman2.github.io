...
export function ExperienceDetail({lang}) {
  ...
  return <>
    ...
    <section className="experience-overview two-col section-pad" data-reveal><div><Eyebrow>01 · OVERVIEW</Eyebrow><h2>{lang==='en'?'What I worked on':'Neler üzerinde çalıştım?'}</h2></div><div><p className="large-copy">{t(e.role,lang)}</p><MethodTags items={e.methods}/></div></section>
    <section className="workflow-band section-pad" data-reveal><SectionHead eyebrow="02 · PROCESS" title={c.labels.workflow}/><div className="process-line">{e.workflow.map((step,i)=><div key={step}><span>{String(i+1).padStart(2,'0')}</span><strong>{step}</strong></div>)}</div></section>
